...
<br>• 서보 모터: 90° (물리 밸브 차단)<br>

<br>• DC 모터: ON (환기 팬 가동)<br>

<br>• LED: ON (경보 표시) | 단일 트리거 래치 적용 |
| `'0'` | `0x30` | **정상 복구 상태**<br>

<br>• 서보 모터: 0° (물리 밸브 개방)<br>

<br>• DC 모터: OFF (환기 팬 정지)<br>

<br>• LED: OFF (경보 해제) | 소프트웨어 인터록 적용 |
...
sequenceDiagram
    ...
    AND->>QT: TCP Connect (Port 8080)
    QT-->>AND: Connection Accepted (Client Count: 1)
    
    loop 100ms 주기 계측
        STM->>QT: UART "1265\n"
        QT->>QT: 라인 파싱 (ADC: 1265 < 3000)
        QT->>QT: 실시간 차트 슬라이딩 윈도우 업데이트
        QT->>AND: TCP "GAS:1265:3000\n"
        AND->>AND: UI 상태 갱신 (NORMAL 배지)
    end

    loop 영상 스트리밍 활성화 시
        AND->>QT: TCP [4B Length][JPEG Binary]
...
sequenceDiagram
    ...
    par 동시 통지 및 제어
        ...
        STM->>STM: 인터록 확인 -> Servo 90° / Fan ON / LED ON
        QT->>AND: TCP "GAS:3250:3000\n"
        AND->>AND: UI 위험 알림 표시 (DANGER 빨간색 전환)
    end
...
sequenceDiagram
    ...
    Note over AND: 관제자가 현장 영상 확인 후 복구 결정
    AND->>QT: TCP "0\n" (밸브 복구 요청)
    QT->>QT: 원격 명령 로그 출력 ("안드로이드 원격 제어: 밸브 복구")
    QT->>STM: UART '0' (복구 명령 전송)
    STM->>STM: Servo 0° (밸브 개방) / Fan OFF / LED OFF
